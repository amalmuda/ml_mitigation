...
Here we build a model to identify ODA agreements targeting climate change mitigation. We build the model using supervised machine learning.
...
Norwegian development aid statistics from 1960 onwards are available to download from [Access to microdata](https://resultater.norad.no/microdata). We select last ten years (2011-2021). These data can also be downloaded by using [this link](https://resultater.norad.no/api/microdata?from_year=2011&to_year=2021&main_region_code=&country_iso_code=&agreement_partner_group_sid=&agreement_partner_sid=&target_area_code=&dac_main_sector_code=&dac_sub_sector_code=&chapter_code=&format=csv&language=en). The csv file is saved in the the *data* folder in the project directory and named *Norad-Norwegian_development_assistance*.
...
Let's create a model specification for the model(s) we want to try. We are specifying three different models:
...
# Fit final model with best tuning parameters
